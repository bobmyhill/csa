...
from models.iuca_no_LRO import R, IUCAModel
...
ss = IUCAModel(cluster_energies, site_species, 3./4.)
...
ss.set_composition_from_p_s([0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5])
print(np.sum(ss.effective_cluster_energies_flat*ss.ideal_cluster_proportions)/R)
print(np.sum(ss.cluster_energies_flat*ss.ideal_cluster_proportions)/R)
exit()
...
for i, T in enumerate(temperatures):
    ...
    Ss[i] = ss.molar_entropy/R/4.
    Ss2[i] = ss2.molar_entropy/R
    Ss3[i] = ss3.molar_entropy/R
...
ax[0].plot(temperatures, Ss2, linestyle='--')
ax[0].plot(temperatures, Ss3, linestyle=':', c='red')
ax[1].plot(temperatures, Es)
ax[1].plot(temperatures, Es2, linestyle='--')
ax[1].plot(temperatures, Es3, linestyle=':', c='red')
ax[2].plot(temperatures, np.gradient(-Gs, temperatures)/R, label='IUCA')
