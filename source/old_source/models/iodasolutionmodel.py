...
class IODAModel(object):
    """
    This is the base class for all
    Isochemical Order-Disorder Approximation models
    """
    ...
    def __init__(self, cluster_energies, site_species,
        ...
        self.W_order = self._ordering_interaction_matrix()
    ...
    def _ordering_interaction_matrix(self):
        N = np.zeros((2, 2, 2, 2, 2, 2, 2, 2))
        for idx, E in np.ndenumerate(self.cluster_energies):
            N[idx[0], idx[1], idx[2], idx[3], :, :, :, :] += E/2.
            N[:, :, :, :, idx[0], idx[1], idx[2], idx[3]] += E/2.

        W = np.zeros((2, 2, 2, 2, 2, 2, 2, 2))
        for idx, n in np.ndenumerate(N):
            if np.sum(np.abs(np.array(idx[:4]) - np.array(idx[4:]))) == 2:
                ...
                ls = [list(set(p)) for p in np.array([idx[:4], idx[4:]]).T]
                es = np.squeeze(self.cluster_energies[np.ix_(*ls)])
                W[idx] += ((es[0, 1] + es[1, 0]) - (es[0, 0] + es[1, 1]))

            #if np.sum(np.abs(np.array(idx[:4]) - np.array(idx[4:]))) == 4:
            #    ls = [list(set(p)) for p in np.array([idx[:4], idx[4:]]).T]
            #    es = np.squeeze(self.cluster_energies[np.ix_(*ls)])
            #    W[idx] += np.min(es) - np.max(es)
    ...
    def set_composition_from_p_s(self, p_s):
        ...
        self.U_ideal = np.sum(self.cluster_proportions
                              * self.cluster_energies_flat)

        p_cl_ground_state = self._ground_state_cluster_proportions(self.p_ind)

        #self.U_order = np.einsum('ij, i, j',
        #                         self.W_order,
        #                         p_cl_ground_state,
        #                         p_cl_ground_state)

        self.U_order = (self.U_ideal
                        - np.sum(p_cl_ground_state
                                 * self.cluster_energies_flat))
    ...
    def maximum_entropy_ps_from_composition(self, c_arr, normalize=True):
