...
class CMFModel(object):
    """
    This is the base class for all Cluster Mean Field models
    """
    ...
    def __init__(self, site_species,
        ...
        self.intracluster_energies = (np.einsum('i, ki',
                                                self.site_species_energies,
                                                self.cluster_occupancies)
                                      + np.einsum('ij, ij, ki, kj->k',
                                                  self.bond_energies,
    ...
    def set_composition_from_p_ind(self, p_ind):
        ...
        self.effective_cluster_energies = (self.intracluster_energies
                                           + np.einsum('ki, i->k',
    ...
    def _cluster_proportions(self, c, temperature):
        lnval = (self.A_ind.dot(c)
                 - (self.effective_cluster_energies / (R * temperature)))
        ...
        if np.max(self._ideal_c) > -1.e-10:  # pure endmember
            ...
            if np.max(np.abs(sol.fun)) < 1.e-8:
                ...
                near_ord_c = (logish(p_cl_near_ord[self.pivots_flat])
                              + (self.effective_cluster_energies[self.pivots_flat]
                                 / (self.temperature * R)))
    ...
    def molar_chemical_potentials(self):
        ...
        g_E = (np.einsum('lk, l->k', D, self.effective_cluster_energies)
    ...
    def molar_internal_energy(self):
        E = np.sum(self.cluster_proportions * self.effective_cluster_energies)
