...
class IUCAModel(object):
    ...
    def __init__(self, cluster_energies, site_species, alpha,
        ...
        self.cluster_permutations = self.permute_sites(degenerate_site_groups,
                                                       self.n_sites)
        dum = np.array(list(range(self.n_sites))) + self.n_sites
        self.W_permutations = np.array([np.concatenate((c, dum))
                                        for c in self.cluster_permutations])
        ...
        self.W_master = self.calculate_master_interaction_matrix()
    ...
    def calculate_master_interaction_matrix(self):
        pslist = []
    ...
    def set_state(self, temperature):
        ...
        self.W = self.interaction_matrix(temperature)
    ...
    def permute_sites(self, site_groups, n_sites):
        degenerate_sites = [idx for site in site_groups for idx in site]
        nondegenerate_sites = [idx for idx in range(n_sites)
                               if idx not in degenerate_sites]
        site_sets = [[nondegenerate_sites]]
        for group in site_groups:
            group_permutations = list(list(p) for p in permutations(group))
            site_sets.append(group_permutations)

        degenerate_clusters = np.array([[s for p in q for s in p]
                                        for q in product(*site_sets)])

        # now we need to put the indices in the correct order
        order_sites = np.array(nondegenerate_sites + degenerate_sites,
                               dtype=int)
        a = np.array(list(enumerate(order_sites)))
        order_sites = a[a[:, 1].argsort()][:, 0]

        ord = degenerate_clusters[:, order_sites]
        return ord[np.lexsort(np.transpose(ord)[::-1])]

    def permute_sites_alpha(self, site_groups, n_sites):
        degenerate_sites = [idx for site in site_groups for idx in site]
        nondegenerate_sites = [idx for idx in range(n_sites)
                               if idx not in degenerate_sites]
        nondegenerate_alphas = ''.join([chr(ord('a')+idx)
                                        for idx in nondegenerate_sites])
        site_sets = [[nondegenerate_alphas]]
        for group in site_groups:
            group_alpha = ''.join([chr(ord('a')+idx) for idx in group])
            group_permutations = [''.join(p)
                                  for p in permutations(group_alpha)]
            site_sets.append(group_permutations)
        degenerate_clusters = np.array([list(''.join(s))
                                        for s in product(*site_sets)])
        ...
        ordered_degenerate_clusters = [''.join(c) for c
                                       in degenerate_clusters[:, order_sites]]

        return sorted(ordered_degenerate_clusters)

    def interaction_matrix(self, temperature):
        exps = np.exp(self.W_master/(R * temperature))
        Eexps = self.W_master * exps
        Es = (np.sum(np.array([np.transpose(Eexps, axes=perm)
                               for perm in self.W_permutations]), axis=0)
              / np.sum(np.array([np.transpose(exps, axes=perm)
                                 for perm in self.W_permutations]), axis=0))
        #Es = self.W_master
        return Es # *self.n_sites
    ...
    def equilibrate_clusters(self):
        if np.max(self._ideal_c) > -1.e-10:  # pure endmember
            ...
            if np.max(np.abs(sol.fun)) < 1.e-8:
                ...
                p_cl_disord = self.ideal_cluster_proportionseffective
