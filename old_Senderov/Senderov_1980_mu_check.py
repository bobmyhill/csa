...
def chempot(ps, T):
    ...
    mu = ideal_mu + nonideal_mu + nonideal_mu_2
    mod = cluster_energies - np.einsum('i, j, k, l, ijkl->', ps[0], ps[1], ps[2], ps[3], mu)
    mu += mod
    return mu
...
for (xb, c) in [#(0, 'red'),
    ...
    for i, y in enumerate(ys):
        ...
        ideal, non_ideal, non_ideal_2 = energy(ps, T)
        ...
        ideal, non_ideal, non_ideal_2 = energy(ps2, T)
        E2 = ideal + non_ideal + non_ideal_2
        dE = (E2 - cluster_energies)
