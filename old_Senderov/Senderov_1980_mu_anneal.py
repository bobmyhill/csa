...
for (xb, c) in [(0, 'red')]: #,
    ...
    for i, y in enumerate(ys):
        ...
        if run_inversion:
            try:
                x = newton_krylov(fns(u, n, Appn, T),
                                  np.exp(logx_approx)+np.random.rand(1)*1.e-5,
                                  method='cgs', f_tol=1.e-8)
            except:
                x = np.exp(logx_approx)+np.random.rand(1)*1.e-5
            ...
            # components of the partition function, phi (Equation 8b)
            pfn = xfn*a
            M1 = pfn/np.sum(pfn) # fractions of different clusters

            facs = np.logspace(0., -0.5, 11)
            for fac in facs:
                try:
                    x = newton_krylov(fns(u, n, Appn, T*fac),
                                      x+np.random.rand(1)*1.e-5,
                                      method='cgs', f_tol=1.e-8)
                except:
                    break
            print('finished')
            ...
            print('max err in cluster proportions: {0:.3f}'.format(np.max(np.abs(M1-M))))
            print(M1)
            print(M)
            #print(M)
            #exit()
        ...
        ideal, non_ideal, non_ideal_2 = energy_components(ps, T*facs[-1])
